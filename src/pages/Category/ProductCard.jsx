...
export function ProductCard({ id, title, logo, price, rating , category , offer}) {
  ...
  return <div dir="ltr" className="relative md:w-64 md:h-80 w-40  h-56 bg-txt rounded-md hover:shadow-lg hover:shadow-btn m-10 transition">
    <Link to={`/shop/${id}`}>
    <div className="flex flex-col md:h-80 h-56 ">
      ...
      <div className="flex md:flex-row flex-col items-center justify-around  md:h-1/4 h-1/2 ">
        <div className=" text-sm  text-black  font-semibold flex flex-col md:items-start items-center justify-center">
          <h2 className="text-btn text-base">{title}</h2>
          <h3>{category}</h3>
        </div>
