              ...
              <div className="flex flex-col items-center gap-10">
                ...
                <div className="flex flex-col items-center gap-4  text-2xl">
                  <div className="flex gap-2 items-center">
                    {
                      data?.is_branch === "0" ? <h2 className="font-semibold">
                        {
                            language ? "Brand" : "علامة تجارية"                          
                        }
                          </h2> : <h2 className="font-semibold">
                            
                            {
                            language ? "Branches" : " فروع"
                            }  </h2>
                    }
                    {/* <a href="#"><FontAwesomeIcon icon={faSortDown} /></a> */}
                  </div>
                  <p> {
                        language ? data?.addresses[0]?.governorate : data?.addresses[0]?.governorate_ar
                    } </p>
                  <p> {
                        language ? data?.addresses[0]?.area : data?.addresses[0]?.area_ar
                    } </p>
                </div>
                <div>
                  <h2 className="font-bold text-2xl">
                    {
                      language ? "Gained Points " : " النقاط المكتسبة "
                    }
                     : <span className="text-btn">{data?.gained_points}</span></h2>
                </div>
              </div>
            </div>
            <div className="swiperr my-8 md:w-4/5 ">
              <Swiper dir="rtl" style={{
